...
obj_edge = 20
point_cloud, last_image = indexhistory(cam,out)
#point_cloud = smoothen_points(point_cloud, tolerance=5)
min_edge = 0
ratio = 0.1

while (min_edge) < obj_edge and (ratio <= 1):  
  ...
  for i in np.arange(15):
    out.write(last_image_curr)
  print("Press SPACE to continue")
  cv2.waitKey(700)
  #cv2.waitKey(0)       

cv2.imwrite('./cv_module/media/shape_before_smooth.png', last_image_curr)
...
if min_edge >= obj_edge: 
  ...
  for i in np.arange(15):
    out.write(last_image_smooth)
  cv2.imwrite('./cv_module/media/shape_smoothed.png', last_image_smooth)
